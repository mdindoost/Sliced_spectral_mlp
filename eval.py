"""
Evaluation script: compare SlicedSpectralMLP (3 weight strategies)
against StandardMLP baselines.

Usage
-----
  python eval.py                   # Cora
  python eval.py --dataset cornell

Output (printed + saved to outputs/eval/)
------------------------------------------
  - Per-slice test accuracy table for all methods
  - outputs/eval/spectral_resolution_curve.png
  - outputs/eval/training_curves_coarse_vs_full.png
"""
...
def _train_sliced(
    ...
    for _ in range(epochs):
        ...
        with torch.no_grad():
            all_logits = model(U)
        coarse_val = _accuracy(all_logits[0],  labels, val_mask)
        full_val   = _accuracy(all_logits[-1], labels, val_mask)
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    for strategy in STRATEGIES:
        ...
        # Per-slice test accuracy (at best val checkpoint)
        model.eval()
        with torch.no_grad():
            all_logits = model(U)
        slice_test_accs = [
            _accuracy(logits, labels, test_mask) for logits in all_logits
        ]
        ...
        print(f"  per-slice test acc:")
        for j, acc in enumerate(slice_test_accs):
            d_j = k // 2 + j
            print(f"    j={j:3d}  d={d_j:3d}  {acc:.4f}")
    ...
    BASELINES = [
        ("StandardMLP-full", k),
        ("StandardMLP-half", k // 2),
    ]
    print("\n[Baselines]")
    for bl_name, n_feat in BASELINES:
        X = U[:, :n_feat]
        bl_model = StandardMLP(
            n_features=n_feat,
            n_classes=n_classes,
            hidden_dim=k,
            n_layers=args.n_layers,
    ...
    for name, r in results.items():
        ...
        print(
            f"{name:<25} {r['best_val']:>9.4f} {full_test:>12.4f} {coarse_test:>14.4f}"
        )
    print(divider)
    ...
    ax.axhline(
        results["StandardMLP-full"]["slice_test_accs"][0],
        color="black", ls="--", lw=1.5, label=f"StandardMLP-full (d={k})",
    )
    ax.axhline(
        results["StandardMLP-half"]["slice_test_accs"][0],
    ...
    plt.tight_layout()
    path = "outputs/eval/spectral_resolution_curve.png"
    ...
    ax.plot(r["val_coarse"], lw=1.5, alpha=0.85,
            label=f"Coarse slice  j=0, d={k//2}")
    ax.plot(r["val_full"],   lw=1.5, alpha=0.85,
            label=f"Full slice   j={k//2}, d={k}")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Validation accuracy")
    ax.set_title(
        f"Coarse vs Full — {args.dataset}, {best_strategy_name}"
    )
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    path = "outputs/eval/training_curves_coarse_vs_full.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"Saved {path}")
